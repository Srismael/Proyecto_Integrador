...
const char* ssid = "Omni_202024";
const char* password = "octo1980";

// Configuración del servidor (reemplaza con la IP de tu servidor Flask)
const char* serverName = "http://172.0.0.1:5000/guardar_uid";


#define TFT_MISO 19  // Si es necesario
#define TFT_MOSI 23
#define TFT_SCLK 18
#define TFT_CS   4   // Pin libre para CS
#define TFT_DC   15  // Pin libre para DC
#define TFT_RST  22  // Pin libre para RST
...
void sendPostRequest(byte *uid, byte uidSize) {
  if (WiFi.status() == WL_CONNECTED) {
    ...
    if (httpResponseCode > 0) {
      ...
      tft.println("Usuario registrado:");
      tft.println(uidString);
    } else {
      Serial.print("Error en la petición POST: ");
      Serial.println(httpResponseCode);
      
      // Mostrar mensaje de error en la pantalla TFT
      tft.fillScreen(TFT_BLACK);
      tft.setCursor(0, 0);
      tft.setTextColor(TFT_RED, TFT_BLACK);
      tft.println("Error en el registro");
    }
    http.end();
  } else {
    Serial.println("Error en la conexión WiFi");
    
    // Mostrar mensaje de error de WiFi en la pantalla TFT
    tft.fillScreen(TFT_BLACK);
    tft.setCursor(0, 0);
    tft.setTextColor(TFT_RED, TFT_BLACK);
